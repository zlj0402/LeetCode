 ...
 * + 【思路 1 -- 写法 2】：动态规划，状态压缩 -- 数组方式
 *      思想是同一个做法，
 *          先给 nums 数组排完序， --> 时间 O(nlogn)
 *          分为两个数组, 分别记住 nums[] 中该值，和该值的和，--> 空间 O(2n)
 *          然后根据 值，进行 DP，状态压缩的值，根据 和 来算；--> 时间 O(U)
 *      + 分析：
 *          + 时间复杂度：排序 O(nlogn) + 整合 O(n) + DP O(U) => O(nlogn)
 *          + 空间复杂度：O(U)
 *      + rank:
 *          + 时间效率：0 ms, 击败 100% （第一下就 0 ms，同时间复杂度，但效率更高一点，主要是因为写法 1，涉及动态内存申请）
 *          + 空间效率：15.64 MB, 击败 41.62%
 *  + 【思路 1 -- 写法 3】：动态规划，状态压缩 -- map
 *      和 -> map[key, value] --> 空间 O(U)
 *      key 单独拿出来排个序 --> 时间 O(UlogU)
 *      DP -> O(U)
 *      + 分析：
 *          + 时间复杂度：整合 O(n) + 排序 O(UlogU) + DP O(U) => O(UlogU)
 *          + 空间复杂度：O(U)
 *      + rank:
 *          + 时间效率：4 ms, 击败 32.69% （按道理来说应该稍快一点，但执行了几次，这是最快的，还是 map 哪里慢了）
 *          + 空间效率：16.41 MB, 击败 29.96%
 */
